...
do $$ begin
 if (select count(*) from public.research_documents)<>0 or (select count(*) from public.research_filing_observations)<>0 then raise exception 'Free/client-metadata bypass'; end if;
 begin perform quantpath_ops.sync_research_from_github();raise exception 'Client sync allowed';exception when insufficient_privilege then null;end;
...
do $$ begin
 ...
 if (select count(*) from public.research_documents)<51 or (select count(*) from public.research_filing_observations)<66 then raise exception 'Paid source read failed'; end if;
...
set local role authenticated;
do $$ begin if (select count(*) from public.research_documents)<>0 or (select count(*) from public.research_filing_observations)<>0 then raise exception 'Expired entitlement bypass';end if;end $$;
reset role;
set local role anon;
