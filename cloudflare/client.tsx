...
function Entry(){const [hash,setHash]=React.useState(location.hash);React.useEffect(()=>{const change=()=>setHash(location.hash);window.addEventListener('hashchange',change);return()=>window.removeEventListener('hashchange',change)},[]);const route=publicRoute(location.pathname);const workspace=hash&&!['#home','#content'].includes(hash);React.useEffect(()=>{if(!workspace)document.documentElement.lang=route?.locale==='zh-hant'?'zh-Hant':'en';},[workspace,route?.locale]);return workspace?<App initialLocale={route?.locale||'en'}/>:route?<PublicSite path={location.pathname}/>:<main style={{padding:40}}><h1>Page not found / 找不到頁面</h1><a href='/en/'>Home / 首頁</a></main>;}
