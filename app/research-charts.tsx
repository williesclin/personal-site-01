import {defaultChart} from './research-layout.mjs';
...
export function ResearchCharts({companies,locale,full,year,chart=defaultChart(),onChart}:{companies:any[];locale:string;full:boolean;year:string;chart?:any;onChart:(v:any)=>void}){
 const zh=locale==='zh-hant',t=(a:string,b:string)=>zh?b:a;
 const {metric,second,mode,sector,selected}=chart;
 const setMetric=(metric:string)=>onChart({...chart,metric}),setSecond=(second:string)=>onChart({...chart,second}),setMode=(mode:string)=>onChart({...chart,mode}),setSector=(sector:string)=>onChart({...chart,sector}),setSelected=(value:any)=>onChart({...chart,selected:typeof value==='function'?value(selected):value});
 ...
 return <section id="charts" className="eq-section"><h2>{t('Build a financial chart','建立多維財報圖表')}</h2><p>{t('Choose up to five companies, a dimension and a chart. Values are company-wide annual facts, not AI-only revenue.','選擇最多五家公司、分析維度與圖表。數字是公司整體年度財報，不是 AI 專屬營收。')}</p>
