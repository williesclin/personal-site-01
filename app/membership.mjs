import {validateJourney} from './research-journey.mjs';
...
export function validateResearchState(value,plan){
 ...
 return {watchlist:[...value.watchlist],saved:value.saved.map(s=>({id:s.id,name:s.name,query:s.query,year:s.year,metrics:[...s.metrics],...(s.journey!==undefined?{journey:validateJourney(s.journey)}:{}),...(s.layout!==undefined?{layout:validateLayout(s.layout)}:{})}))};
}
