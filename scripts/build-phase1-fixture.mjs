...
const out=process.env.QP_FIXTURE_OUTPUT||'/tmp/quantpath-phase1-fixture';await fs.mkdir(out,{recursive:true});
await build({configFile:false,resolve:{alias:{'@':process.cwd()}},define:{'process.env.NODE_ENV':'"production"'},build:{outDir:out,emptyOutDir:false,lib:{entry:'tests/phase1-fixture.tsx',name:'Phase1Fixture',formats:['iife'],fileName:()=> 'fixture.js'},minify:true}});
